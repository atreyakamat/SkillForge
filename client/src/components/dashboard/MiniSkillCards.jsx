    ...
    <div className="space-y-3">
      <div className="flex items-center justify-between mb-3">
        ...
        <div className="flex items-center text-xs text-gray-500">
          ...
          {recentActivity.reduce((sum, skill) => sum + skill.timeSpent, 0)} min
        </div>
      </div>
        ...
        {recentActivity.map((skill) => {
          ...
          return (
            <div
              ...
              className={`${colors.bg} ${colors.border} border rounded-lg p-3 transition-all duration-200 hover:scale-105 hover:shadow-md cursor-pointer magical-card group`}
            >
      ...
      <div className="mt-4 p-3 bg-gradient-to-r from-blue-50 to-purple-50 rounded-lg border border-blue-100">
        <div className="text-xs text-gray-600 mb-1">Yesterday's Summary</div>
        <div className="flex justify-between text-sm">
          <span className="text-gray-700">
            {recentActivity.length} skills practiced
          </span>
          <span className="text-blue-600 font-medium">
            +{recentActivity.reduce((sum, skill) => sum + skill.progressGained, 0)} total points
          </span>
        </div>
      </div>
      ...
      <button 
        onClick={() => navigate('/progress')}
        className="w-full mt-3 px-3 py-2 text-sm bg-white border border-gray-200 rounded-lg hover:bg-gray-50 hover:border-gray-300 transition-colors flex items-center justify-center space-x-2"
      >
        <TrendingUp className="w-4 h-4 text-blue-500" />
        <span className="text-gray-700">View Full Progress</span>
      </button>
    </div>
  )
}
